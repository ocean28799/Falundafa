  ...
  switch (type) {
    case DISPATCHES.SET_CURRENT_SONG:
      ...
      return {
        ...
        currentSong: {
          playback:
            config?.playback === "current"
              ? state?.currentSong?.playback
              : payload?.playback,
          soundObj:
            config?.soundObj === "current"
              ? state?.currentSong?.soundObj
              : payload?.soundObj,
          detail:
            config?.detail === "current"
              ? state?.currentSong?.detail
              : payload?.detail,
          playbackStatus:
            config?.playbackStatus === "current"
              ? state?.currentSong?.playbackStatus
